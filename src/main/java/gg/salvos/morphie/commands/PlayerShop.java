			...
			if (s.hasPermission("mshops.ps")) {
				...
				if (args.length == 0) {
					...
					if (args[0].equalsIgnoreCase("set")) {
						...
						if (s.hasPermission("mshops.admin") || s.hasPermission("mshops.reload")) {
					          Plugin pl = Bukkit.getServer().getPluginManager().getPlugin("MorphShopsPremium");
					          if (pl != null) {
					            this.plugin.reloadConfig();
				            	plugin.getServer().getPluginManager().disablePlugin(plugin);
				            	plugin.getServer().getPluginManager().enablePlugin(plugin);
					            s.sendMessage(ChatColor.translateAlternateColorCodes('&', new MessagesManager(plugin).getMessage("Prefix") + new MessagesManager(plugin).getMessage("ReloadMessage")));
					            return true;
					          }
						} else {
							s.sendMessage(ChatColor.translateAlternateColorCodes('&', new MessagesManager(plugin).getMessage("ErrorPrefix") + new MessagesManager(plugin).getMessage("NoPermission")));
